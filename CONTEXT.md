...
- **Escape na sintaxe Intlify (unplugin-vue-i18n):** o caractere `@` inicia _linked message_. Para exibir arroba literal (e-mails, handles), no JSON use **barra invertida dupla** antes do `@`: por exemplo `"werlesson\\@email.com"` — o compilador gera `werlesson@email.com` na UI. Sem esse escape, o build falha com _Invalid linked format_.
